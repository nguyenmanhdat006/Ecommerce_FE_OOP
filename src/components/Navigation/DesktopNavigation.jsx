...
export default function DesktopNavigation({ links, actions }) {
  return (
    <>
      {/* Navigation - desktop */}
      <nav className="hidden md:flex gap-10 text-lg font-medium text-muted-foreground">
        {links.map((link, i) => (
          ...
          >
            {link.text}
            <span className="absolute left-0 bottom-[-6px] h-[3px] w-0 bg-primary rounded-full transition-all duration-300 group-hover:w-full" />
          </a>
        ))}
      </nav>
