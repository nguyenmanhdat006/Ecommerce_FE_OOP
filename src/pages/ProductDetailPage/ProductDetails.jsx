...
const ProductDetails = () => {
  ...
  const addItemToCart = useCallback(async ()=>{
    ...
    // find variant by selected size (fallback to first)
    const variant = product?.variants?.find(v => v.size === selectedSize) || product?.variants?.[0] || null;
    ...
    try{
      ...
      // TODO: rollback optimistic update if desired
    }
  ...
  return (
    ...
    <div className='flex flex-col md:flex-row px-10'>
      ...
      <div className='w-[60%] px-10'>
        ...
        <div className='mt-2'>
          <SizeFilter sizes={sizes} hidleTitle multi={false} onChange={(v) => setSelectedSize(v?.[0] ?? null)} />
        </div>
        <div className='flex items-center gap-2 py-2'>
          <label className='text-sm'>Quantity</label>
          <input type='number' min={1} value={quantity} onChange={(e) => setQuantity(Math.max(1, Number(e.target.value || 1)))} className='w-[80px] border rounded px-2 py-1' />
        </div>
        <div>
        ...
        <div className='flex py-4'>
          {(() => {
            ...
            return (
              <button className='bg-gray-300 rounded-lg px-4 py-2' disabled>In cart</button>
            )
          })()}
        </div>
